...
- The release matrix now uses the repository's real `aarch64-darwin` runner label and cross-compiles `aarch64-linux` with stable Rust plus the GNU cross libc/toolchain on a self-hosted ephemeral x86_64 runner, because no native aarch64 Linux Actions runner is registered (bd-11731d).
