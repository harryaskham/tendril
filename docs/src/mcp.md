...
The `capture` tool accepts an optional `camera` argument (mutually exclusive with `window`/`display`) to grab a single frame from a video capture device discovered via the `list` tool's `cameras` array. On macOS this uses ffmpeg's AVFoundation backend; other platforms return a structured unsupported-capability error.
